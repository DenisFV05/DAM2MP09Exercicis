...
app.post('/api/admin/restart-match', (req, res) => {
  if (!adminPassword) {
    return res.status(503).json({ ok: false, error: 'WEB_ADMIN_PASSWORD is not configured.' });
  }
...
const httpServer = app.listen(port, () => {
    console.log(`🎮 BattleRoyale Server escoltant a: http://localhost:${port}`);
});
...
ws.onMessage = (socket, id, msg) => {
    if (debug) console.log(`New message from ${id}: ${msg.substring(0, 64)}...`);
...
function shutDown() {
  ...
  shuttingDown = true;
  console.log('Aturant el servidor...');
  httpServer.close(() => {
    ws.end();
    gameLoop.stop();
    process.exit(0);
...
function hasValidAdminSecret(req) {
  ...
  return candidates.some(c => secretsMatch(c, adminPassword));
}
